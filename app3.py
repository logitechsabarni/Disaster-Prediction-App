...
def get_mock_weather(city: str) -> Tuple:
    ...
    20.5937,   # FIXED LAT (India)
    78.9629,   # FIXED LON (India)
...
def show_flood_risk():
    ...
    if st.button("🔍 Run Flood Assessment", use_container_width=True):
        if "," not in city:
            st.warning("⚠️ Please enter city with country code (e.g., Patna, IN)")
            return
        if not city:
            ...
            with st.spinner("🔄 Fetching meteorological data and analyzing hydrological risk..."):
                ...
                if weather_data is None:
                    ...
                    if rainfall > 40:
                        ...
                        risk_level = 0   # Low if rainfall < 20 else (np.random.randint(1, 3) if rainfall < 40 else 2)
